...
/// <summary>
/// 冰霜炮（稀有）：能耗5 / 上限5 / 射速1秒。
/// 向最近敌人发射快速直线冰弹，伤害并施加 3 秒 [寒冷]。
/// </summary>
public class FrostCannonModule : ModuleBase
{
    ...
    public override ModuleType ModuleType => global::ModuleType.FrostCannon;
    ...
    void ApplyLevelStats(int level)
    {
        ...
        damage = ModuleCatalog.GetFrostCannonDamage(lvl);
        energyCapacity = ModuleCatalog.GetFrostCannonEnergyCapacity(lvl);
        energyPerShot = ModuleCatalog.GetFrostCannonEnergyPerShot(lvl);
        fireInterval = ModuleCatalog.GetFrostCannonFireInterval(lvl);
    ...
    void Update()
    {
        ...
        _cooldown = interval;
        var go = new GameObject("FrostCannonProjectile");
        var proj = go.AddComponent<FrostCannonProjectile>();
        proj.Launch(transform.position, target, damage, this);
        RefreshVisual();
    }
    ...
    public override void RefreshVisual()
    {
        ...
        if (show && _energyHudFill != null)
        {
            ...
            _energyHudFill.color = new Color(0.5f, 0.9f, 1f, 1f);
        }
